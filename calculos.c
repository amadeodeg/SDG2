...
#define NUM_MUESTRAS_CALCULO 80 //Ventana de calculo de DFT
#define	NIVELES 32
#define NUM_COMPR_NEC 5  //log2 (NIVELES)
#define NUM_MUESTRAS_DESFASE NUM_MUESTRAS_PERIODO_10HZ/4
#endif 
...
//Actualiza los punteros para el uso en la DFT
void 
actualizaPunteros(int i)
{ 
	pSin[i] += paso_frecs[i];
	pCos[i] += paso_frecs[i];
...
//Calcula el salto de muestras para cada frecuencias. Se llama una vez al principio.
void 
configPasoFrecPointers()
{
	...
	for (i = 0; i < NUM_FREC_MUESTREADAS; i++)
	{
		...
		pCos[i] = sinusoide10Hz+NUM_MUESTRAS_DESFASE;
	}
}

//Calcula la DFT de una ventana de muestras de tamaño dado (NUM_MUESTRAS_CALCULO). 
inline void 
calculaModuloDFT(int muestraADC)
{
	int i;
	for (i = 0; i < NUM_FREC_MUESTREADAS; i++)
	{
	...
	if (numMuestrasLeidas >= NUM_MUESTRAS_CALCULO){
		int i;
		for(i = 0; i < NUM_FREC_MUESTREADAS ;i++){
			modFrecCos[i] = modFrecCos[i]/1024;   //Desescalado
...
//Convierte el modulo en valores acordes con el DAC de antares, para evitar tener que calcular 
//logaritmos.
...
//Devuelve el puntero al array con los modulos de las diferentes frecuencias.
int* 
getPmodFrecTot(void)
{
	return modFrecTot;
...
inline int calculaSeno(int frecuencia, int muestra){
	return sinusoide10Hz[paso_frecs[frecuencia]*muestra%NUM_MUESTRAS_PERIODO_10HZ];
}
