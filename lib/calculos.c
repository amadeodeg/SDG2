...
void calculaModuloDFT(int muestraADC){
	...
	for (i = 0; i < NUM_FREC_MUESTREADAS; i++){
		...
		if (DEBUG) printf("Muestra: %d Seno: %d Coseno: %d\n", i, modFrecSen[i], modFrecCos[i]);
		modFrecCos[i]=modFrecCos[i]+muestraADC*calculaCoseno(frecuenciasMuestreo[i],i);
		modFrecSen[i]=modFrecSen[i]+muestraADC*calculaSeno(frecuenciasMuestreo[i],i);
		if (DEBUG) printf("Muestra: %d Seno: %d Coseno: %d\n", i, modFrecSen[i], modFrecCos[i]);
	}

	if (DEBUG) printf("Seno1: %d Coseno1: %d\n", modFrecSen[1], modFrecCos[1]);
	...
	if (numMuestrasLeidas>=NUM_MUESTRAS_CALCULO){
		int i;
		if (DEBUG) printf("He leido 80 muestras\n");
		if (DEBUG) printf("Seno1: %d Coseno1: %d\n", modFrecSen[1], modFrecCos[1]);
		for(i=0;i<NUM_FREC_MUESTREADAS;i++){
			if (DEBUG) printf("Frecuencia: %d Modulo: %d Seno: %d Coseno %d\n",i,modFrecTot[i], modFrecCos[i], modFrecSen[i]);
